...
def main() -> None:
    ...
    grid = {
        "beta": [0.079, 0.08, 0.081, 0.082],
        "tau_r": [10, 11, 12],
        "r_max": [525.0, 550.0, 575.0],
        "c_max": [375.0, 400.0, 425.0],
        "k_c": [0.0025, 0.0030, 0.0035],
        "tau_q": [58, 60, 62],
        "q_max": [0.06, 0.07, 0.08],
        "k_q": [0.06, 0.08, 0.10],
    }
    ...
    for params in itertools.product(
        ...
        beta, tau_r, r_max, c_max, k_c, tau_q, q_max, k_q = params
    ...
    with Path("final_fine_calibration_results.csv").open("w", encoding="utf-8-sig", newline="") as f:
    ...
    with Path("final_fine_best_summary.txt").open("w", encoding="utf-8") as f:
        f.write("best_params\n")
        for key in out_fields:
            f.write(f"{key}={best[key]}\n")

    with Path("final_fine_best_simulation.csv").open("w", encoding="utf-8-sig", newline="") as f:
        writer = csv.writer(f)
