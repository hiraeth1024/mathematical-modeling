...
def simulate(
    ...
    for t in range(NUM_DAYS + 1):
        ...
        bt = 0.0 if t < 15 else min(25.0 * (t - 15), 300.0)
...
def build_loss(sim_prices: dict[str, float], obs_prices: dict[str, float]) -> tuple[float, dict[str, float]]:
    ...
    tail_dates = ["2026-05-28", "2026-05-29"]
    ...
    loss = base_sse + 4.0 * peak_err + 6.0 * mid_err + 8.0 * late_err + 6.0 * tail_err + 2.0 * jump_err
    return loss, {
        "sse": base_sse,
        "peak": peak,
        "mid_mean": mid_mean,
...
def main() -> None:
    ...
    for vals in combos:
        ...
        if not (1600.0 >= l2 >= l3):
            continue
        if not (t1 < t2 and t_switch > t_hold):
            continue
        if k2 <= k1:
            continue

        sim_prices = simulate(
            beta, tau_r, r_max, c_max, k_c, l2, l3, t1, t2, f0, t_hold, t_switch, k1, k2
        )
    ...
    out_fields = [
        "beta", "tau_r", "r_max", "c_max", "k_c",
        "l2", "l3", "t1", "t2",
        "f0", "t_hold", "t_switch", "k1", "k2",
        "sse", "peak", "mid_mean", "late_mean", "jump_pct", "tail_err", "loss",
    ]
    with Path("joint_two_stage_rebalance_results.csv").open("w", encoding="utf-8-sig", newline="") as f:
    ...
    best_prices = simulate(
        ...
        best["l2"], best["l3"], best["t1"], best["t2"],
        best["f0"], best["t_hold"], best["t_switch"], best["k1"], best["k2"]
    )

    with Path("joint_two_stage_rebalance_best_summary.txt").open("w", encoding="utf-8") as f:
        f.write("best_params\n")
        for key in out_fields:
            f.write(f"{key}={best[key]}\n")

    with Path("joint_two_stage_rebalance_best_simulation.csv").open("w", encoding="utf-8-sig", newline="") as f:
        writer = csv.writer(f)
    ...
    print(
        "best="
        f"beta={best['beta']},tau_r={best['tau_r']},r_max={best['r_max']},c_max={best['c_max']},k_c={best['k_c']},"
        f"l2={best['l2']},l3={best['l3']},t1={best['t1']},t2={best['t2']},"
        f"f0={best['f0']},t_hold={best['t_hold']},t_switch={best['t_switch']},k1={best['k1']},k2={best['k2']}"
    )
