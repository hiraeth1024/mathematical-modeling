...
def main() -> None:
    ...
    grid = {
        "beta": [0.08, 0.082, 0.085],
        "tau_r": [10, 11],
        "r_max": [550.0, 600.0, 650.0],
        "c_max": [400.0, 425.0, 450.0],
        "k_c": [0.002, 0.0025, 0.003],
        "tau_q": [45, 50, 55, 60],
        "q_max": [0.03, 0.05, 0.07, 0.09],
        "k_q": [0.08, 0.12, 0.16],
    }

    results = []
    for beta, tau_r, r_max, c_max, k_c, tau_q, q_max, k_q in itertools.product(
        grid["beta"], grid["tau_r"], grid["r_max"], grid["c_max"], grid["k_c"],
        grid["tau_q"], grid["q_max"], grid["k_q"]
    ):
        sim_prices = simulate(beta, tau_r, r_max, c_max, k_c, tau_q, q_max, k_q)
    ...
    out_fields = ["beta", "tau_r", "r_max", "c_max", "k_c", "tau_q", "q_max", "k_q", "sse", "peak", "mid_mean", "late_mean", "jump_pct", "tail_err", "loss"]
    with Path("late_demand_destruction_results.csv").open("w", encoding="utf-8-sig", newline="") as f:
    ...
    best_prices = simulate(
        ...
        best["tau_q"], best["q_max"], best["k_q"]
    )

    with Path("late_demand_destruction_best_summary.txt").open("w", encoding="utf-8") as f:
        f.write("best_params\n")
        for key in out_fields:
            f.write(f"{key}={best[key]}\n")

    with Path("late_demand_destruction_best_simulation.csv").open("w", encoding="utf-8-sig", newline="") as f:
        writer = csv.writer(f)
    ...
    print(
        ...
        f"tau_q={best['tau_q']},q_max={best['q_max']},k_q={best['k_q']}"
    )
