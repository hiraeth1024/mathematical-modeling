...
def cooling_window(t: int, amp: float, mu: float, sigma: float) -> float:
    return amp * math.exp(-((t - mu) ** 2) / (2.0 * sigma ** 2))


def ft_base(t: int) -> float:
    if t < 10:
        return F0
...
def simulate(
    ...
    for t in range(NUM_DAYS + 1):
        ...
        ft = ft_base(t) * (1.0 - cooling_window(t, cool_amp, cool_mu, cool_sigma))
...
def build_loss(sim_prices: dict[str, float], obs_prices: dict[str, float]) -> tuple[float, dict[str, float]]:
    ...
    loss = base_sse + 4.0 * peak_err + 6.0 * mid_err + 10.0 * late_err + 8.0 * tail_err + 2.0 * jump_err + 6.0 * dip_err
    return loss, {
        "sse": base_sse,
        "peak": peak,
        "mid_mean": mid_mean,
...
def main() -> None:
    ...
    grid = {
        ...
        "q_max": [0.07, 0.08],
        "k_q": [0.08, 0.10],
        "cool_amp": [0.05, 0.08, 0.10, 0.12],
        "cool_mu": [39.0, 41.0, 43.0, 45.0],
        "cool_sigma": [2.0, 3.0, 4.0, 5.0],
    }

    results = []
    for vals in itertools.product(
        grid["beta"], grid["tau_r"], grid["r_max"], grid["c_max"], grid["k_c"],
        grid["tau_q"], grid["q_max"], grid["k_q"], grid["cool_amp"], grid["cool_mu"], grid["cool_sigma"]
    ):
        beta, tau_r, r_max, c_max, k_c, tau_q, q_max, k_q, cool_amp, cool_mu, cool_sigma = vals
        sim_prices = simulate(beta, tau_r, r_max, c_max, k_c, tau_q, q_max, k_q, cool_amp, cool_mu, cool_sigma)
    ...
    out_fields = [
        ...
        "cool_amp", "cool_mu", "cool_sigma", "sse", "peak", "mid_mean",
        "late_mean", "jump_pct", "tail_err", "dip_err", "loss"
    ]

    with Path("f_cooling_results.csv").open("w", encoding="utf-8-sig", newline="") as f:
    ...
    best_prices = simulate(
        ...
        best["tau_q"], best["q_max"], best["k_q"], best["cool_amp"], best["cool_mu"], best["cool_sigma"]
    )

    with Path("f_cooling_best_summary.txt").open("w", encoding="utf-8") as f:
        f.write("best_params\n")
        for key in out_fields:
            f.write(f"{key}={best[key]}\n")

    with Path("f_cooling_best_simulation.csv").open("w", encoding="utf-8-sig", newline="") as f:
        writer = csv.writer(f)
    ...
    print(
        ...
        f"cool_amp={best['cool_amp']},cool_mu={best['cool_mu']},cool_sigma={best['cool_sigma']}"
    )
